...
# 3. permutation on the cort_ind ~ network_centrality models
## load the models
m_ind <- readRDS("models/m_ind.rds")

## the betwenness model
orig_b <- data.frame(term = c("(Intercept)","betw","sexm","ageAD","barl_ind"), estimate = coef(m_ind$betw))

net_stress %>%
  filter(!is.na(betw)) %>%
  group_by(perm) %>%
  do(tidy(glm(cort_ind + 0.0001 ~ betw + sex + age + barl_ind, .[-272,], family = Gamma(link = "log")))) %>%
  left_join(orig_b, by = "term") %>%
  group_by(term) %>%
  summarise(p_perm = sum(estimate.x > estimate.y) / n_perm) %>%
  mutate(p_perm = ifelse(p_perm > 0.5, 1 - p_perm, p_perm)) -> dd_betw
...
## the eigen model
orig_e <- data.frame(term = c("(Intercept)","eigen","sexm","ageAD","barl_ind"), estimate = coef(m_ind$eigen))

net_stress %>%
  filter(!is.na(betw)) %>%
  group_by(perm) %>%
  do(tidy(glm(cort_ind + 0.0001 ~ eigen + sex + age + barl_ind, .[-c(25,272,279),], family = Gamma(link = "log")))) %>%
  left_join(orig_e, by = "term") %>%
  group_by(term) %>%
  summarise(p_perm = sum(estimate.x > estimate.y) / n_perm) %>%
  mutate(p_perm = ifelse(p_perm > 0.5, 1 - p_perm, p_perm)) -> dd_eigen
...
## the degree model
orig_d <- data.frame(term = c("(Intercept)","deg","sexm","ageAD","barl_ind"), estimate = coef(m_ind$degree))

net_stress %>%
  filter(!is.na(betw)) %>%
  group_by(perm) %>%
  do(tidy(glm(cort_ind + 0.0001 ~ deg + sex + age + barl_ind, .[-272,], family = Gamma(link = "log")))) %>%
  left_join(orig_d, by = "term") %>%
  group_by(term) %>%
  summarise(p_perm = sum(estimate.x > estimate.y) / n_perm) %>%
  mutate(p_perm = ifelse(p_perm > 0.5, 1 - p_perm, p_perm)) -> dd_degree
...
## the strength model
orig_s <- data.frame(term = c("(Intercept)","strength","sexm","ageAD","barl_ind"), estimate = coef(m_ind$strength))

net_stress %>%
  filter(!is.na(betw)) %>%
  group_by(perm) %>%
  do(tidy(glm(cort_ind + 0.0001 ~ strength + sex + age + barl_ind, .[-272,], family = Gamma(link = "log")))) %>%
  left_join(orig_s, by = "term") %>%
  group_by(term) %>%
  summarise(p_perm = sum(estimate.x > estimate.y) / n_perm) %>%
  mutate(p_perm = ifelse(p_perm > 0.5, 1 - p_perm, p_perm)) -> dd_strength

# 4. the feeding frequency ~ network centrality models
## load the models
m_feed <- readRDS("models/m_feed.rds")

## betwennness
orig_b <- data.frame(term = c("(Intercept)","betw","sexm","ageAD"), estimate = coef(m_feed$betw))

net_stress %>%
  filter(!is.na(betw)) %>%
  group_by(perm) %>%
  do(tidy(lm(feeding_freq ~ betw + sex + age,.))) %>%
  left_join(orig_b, by = "term") %>%
  group_by(term) %>%
  summarise(p_perm = sum(estimate.x > estimate.y) / n_perm) %>%
  mutate(p_perm = ifelse(p_perm > 0.5, 1 - p_perm, p_perm)) -> dd_betw
...
## eigen
orig_e <- data.frame(term = c("(Intercept)","eigen","sexm","ageAD"), estimate = coef(m_feed$eigen))

net_stress %>%
  filter(!is.na(betw)) %>%
  group_by(perm) %>%
  do(tidy(lm(feeding_freq ~ eigen + sex + age,.))) %>%
  left_join(orig_e, by = "term") %>%
  group_by(term) %>%
  summarise(p_perm = sum(estimate.x > estimate.y) / n_perm) %>%
  mutate(p_perm = ifelse(p_perm > 0.5, 1 - p_perm, p_perm)) -> dd_eigen
...
## degree
orig_d <- data.frame(term = c("(Intercept)","deg","sexm","ageAD"), estimate = coef(m_feed$degree))

net_stress %>%
  filter(!is.na(betw)) %>%
  group_by(perm) %>%
  do(tidy(lm(feeding_freq ~ deg + sex + age,.))) %>%
  left_join(orig_d, by = "term") %>%
  group_by(term) %>%
  summarise(p_perm = sum(estimate.x > estimate.y) / n_perm) %>%
  mutate(p_perm = ifelse(p_perm > 0.5, 1 - p_perm, p_perm)) -> dd_degree
...
## strength
orig_s <- data.frame(term = c("(Intercept)","strength","sexm","ageAD"), estimate = coef(m_feed$strength))

net_stress %>%
  filter(!is.na(betw)) %>%
  group_by(perm) %>%
  do(tidy(lm(feeding_freq ~ strength + sex + age,.))) %>%
  left_join(orig_s, by = "term") %>%
  group_by(term) %>%
  summarise(p_perm = sum(estimate.x > estimate.y) / n_perm) %>%
  mutate(p_perm = ifelse(p_perm > 0.5, 1 - p_perm, p_perm)) -> dd_strength
